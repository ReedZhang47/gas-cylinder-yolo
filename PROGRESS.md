...
> - **A/B 两臂仍未出**（将并入 L1/L2 一次训练）：A 臂 2026-09-19 00:12 误启动已中断；B 臂增广脚本 `make_aug93_dataset.py` 尚未写。
> - **runs 已整理**：12.2 GB → **0.48 GB**；三批快照 11.06 GB 经用户特许物理删除，四个结果 JSON 的 SHA256 留档于 `logs\p1_pilot_results.sha256.txt`；试点 run 归档 `_trash\runs\`。
> - **耗时模型已实测校准**（见 `EXPERIMENTS.md` 五节）：每图每轮 9.8~23.2 ms（s 级 ≈10、m 级 ≈20）→ **1085 张 × 300 轮 = 53~126 min/权重**；**L1+L2（A+B+C × 六权重）≈ 18.25 h**、规模曲线 493 点 ≈ 4.0 h，合计 **≈22.3 h**。（早先"4~6 h / 9.3 h"两个估算分别低估约 3 倍与 2 倍，已被实测推翻。）
...
- **run 目录**（`runs\detect\`，已整理至 0.48 GB）：`gen1085\`（C 臂六权重，仅留 `last.pt`）、`gen493v3\`（规模曲线第一点）、`v3\`（评测输出）、`v3_check\`（体检）。`best.pt`（6 个，189 MB）与三次试点的 run 已归档 `_trash\runs\`；三批快照（11.06 GB）经用户特许物理删除，结果 JSON 的 SHA256 留在 `logs\p1_pilot_results.sha256.txt`。
...
- **踩坑（含 v1/v2 教训，v3 继续适用）**：
  ...
  8. **100 轮不是"跑够了"，但 300 轮够用**：100 轮时自评末段仍在涨；300/450/750 三次试点显示独立集合在 **400 轮后无系统增益** → **今后统一 300 轮**（见 `EXPERIMENTS.md` 二节）。凡做臂间/预算间比较必须**同预算**，并显式声明 `close_mosaic=10` 的占比差异（300 轮 3.3% vs 100 轮 10%）。
  ...
  13. **快照目录很占空间**：三次试点共产生 150 个快照（约 11 GB）。**2026-09-20 已按用户特许物理删除**（结论与原始 JSON 均已留档：`runs\detect\p1_*.json`、`pilot_saturation*.json`，SHA256 见 `logs\p1_pilot_results.sha256.txt`）。今后跑快照实验：先确认 JSON 落盘，再删快照；`_trash` 放不下这一量级。
...
| `EXPERIMENTS.md` | **实验总纲**：三条实验线（主线三臂 / P2 协议验证 / 支撑试点）的区分与设计、统一协议、耗时模型 |
