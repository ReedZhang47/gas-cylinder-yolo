...
- **协议讨论记录**：现行的"61test 方案"（各臂划 val 自评 + 固定轮数报 last.pt + 61 张独立 test）与候选的"61val 方案"（**三臂全量训练** + 61 张网络图当 val 选权 + 报该 val 成绩 + B 臂增广量对齐 1085）的对照、双方论证、P1 实测与 P2 计划，已整理为根目录 `PLAN_61VAL.md`；实验登记在 `List_of_Experiments.md` 二·五节。两方案**并存**（61val 不替代 61test：泛化/域差读数只能由未被动过的 test 给出）。
...
- **踩坑（含 v1/v2 教训，v3 继续适用）**：
  ...
  10. **检查点选择的方差远大于臂间差**：P1 实测同一训练 30 个快照在 61 张上的 mAP50-95 极差 0.0003~0.4487、**std 0.1396**。→ 任何"选哪个检查点"的规则都会引入比实验处理更大或相当的方差；报数时必须同时给出选择规则与选权集合，否则数字不可比。核对快照时注意 `runs\detect\_p1_snapshots\`（约 2.6 GB，P1 完成后可删）。
