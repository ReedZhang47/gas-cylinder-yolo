...
- **饱和试点（2026-09-20，450 轮，详见 `PLAN_61VAL.md` 六节）**：与 P1 同设置（A 臂 93 张 + yolo26s、`patience=0`、每 10 轮快照），总轮数 300→**450**。① **仍未饱和**：61 张上 100→200 **+0.1360**、200→300 +0.0026、300→400 **+0.0435**，平滑曲线到 420 轮仍创新高；② 末轮 0.4684 vs 最优快照（ep430）0.5259 → 选权增益 **+0.0575**（与 P1 的 +0.0549 复现）；③ **自评 val 在 400~450 段已走平甚至下降（−0.0040），而同段 61 张还在涨 → 不能用自评曲线判断"训够了没有"**；④ **重训噪声首次量化**：P1 与本次两次独立训练在共享 epoch 上逐点差 std **0.0670**、中位绝对差 **0.0356**（均值仅 −0.0047）。→ 主表预算需先做 450→750 延长试点；报数必须给噪声量级。
...
- **踩坑（含 v1/v2 教训，v3 继续适用）**：
  ...
  11. **重训噪声的实测量级**：两次同设置独立训练（P1 300 轮 vs 饱和试点 450 轮）在共享 epoch 上逐点差 **std 0.067、中位绝对差 0.036、极差 −0.23~+0.12**，而整体趋势一致（均值 −0.005）。→ **单次读数不可作准**：臂间差值（当前 +0.07~+0.15）虽大于噪声中位差，但同量级，必须靠选权/多轮平均/多种子压低，并把噪声量级写进论文。
  12. **不能用自评 val 曲线判断"训够了没有"**：450 轮试点中自评在 400~450 段走平甚至下降，同段 61 张上仍在涨（100→200 +0.136、300→400 +0.044）。判饱和只能看独立集合，或加预算后重测。
  13. **快照目录很占空间**：`runs\detect\_p1_snapshots\`（30 个，2.19 GB）与 `runs\detect\_pilot_sat\snapshots\`（45 个，约 3.3 GB）在对应实验结论落定后可删；删前确认 `p1_*.json` / `pilot_saturation.json` 已存。
