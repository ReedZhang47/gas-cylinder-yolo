...
- **协议依据：为什么报 last.pt 而不是 val 选权（2026-09-20 实测）**：把 gen1085 六权重训练时存下的 `best.pt`（在自评 val 217 张上选出）与 `last.pt` 放在同一个 test61 上对比 —— **5/6 权重 best.pt 更差，mAP50-95 平均 −0.0252**（最差 −0.0723；仅 yolo26s +0.0044 略优），mAP50 平均 −0.0210。原因是自评 val 与训练集同源（同一批生成图，甚至共享源照片），在其上"更优"偏向过拟合合成域，而非在独立真实场景更强。→ **固定轮数报 last.pt 不只是"公平"，在这批数据上是更准的选择**，该结论可写进论文当协议依据。
...
4. 方法学补充：复核价值消融、负样本消融、多随机种子、工作点 PR、误差分析图（见 `List_of_Experiments.md`）。其中**多随机种子（末轮取均值/中位数）**是提升指标稳健性的正路（取代 val 选权，见五节坑 7）。
5. **可选：补一个独立 test 扩充**（再收 40~60 张网络图，保持与源数据无关），把 test 从 61 张/72 框扩到 ~120 框量级以收紧置信区间；原有 61 张转作固定 benchmark 或 dev 集。优先级中（见 `List_of_Experiments.md`）。
6. 多目标扩展：安全帽（第二目标）。
7. 论文：按 v3 口径改写 Methodology（编辑合成 = 状态级增广）与 Experiments；投稿策略见 `paper\PAPER_PLAN.md`。
...
7. **别用自评 val 选权**（`best.pt`）：实测见三节"协议依据"——在合成域 val 上选出的检查点在独立 test 上平均更差（−0.025 mAP50-95）。要更好的指标应走"多种子 × 末轮取均值"，而不是换选权规则。
