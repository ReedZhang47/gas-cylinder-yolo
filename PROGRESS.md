...
- **协议讨论记录**：现行的"61test 方案"（各臂划 val 自评 + 固定轮数报 last.pt + 61 张独立 test）与候选的"61val 方案"（**三臂全量训练** + 61 张网络图当 val 选权 + 报该 val 成绩 + B 臂增广量对齐 1085）的对照、双方论证与判别实验 P1，已整理为根目录 `PLAN_61VAL.md`；实验登记在 `List_of_Experiments.md` 二·五节。两方案**并存**（61val 不替代 61test：泛化/域差读数只能由未被动过的 test 给出）。
