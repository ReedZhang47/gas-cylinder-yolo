...
**图表素材清单（2026-09-23 更新）**：图 1 管线总图（工作区外绘制，占位已可编译）；图 2 三臂样例对比（脚本出，去留未定，26 MB 待压）；图 3 选权曲线（已升级为三臂，脚本出）；图 4 主结果 CI 图（脚本出，待接线）；选权表（脚本出，已覆盖三臂）；三臂主表（fixed endpoint + pooled OOF，两指标）待生成；数据构成 / 生成图来源审计 / 来源独立性 → 作为正文表与数字，不做图；规模曲线（493 vs 1085）待 gen493；**定性检出图待补（需框坐标，见 `NEXT_STEPS.md`）**；【待】安全帽样本与结果。
