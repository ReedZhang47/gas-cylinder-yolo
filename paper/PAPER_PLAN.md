...
> 当前口径（2026-09-25）：v4 的 A/B/C 三臂与 493 张规模实验已完成，61 张外部图称 **dev61**，不是 sealed final test。v5 新增 real93 LoRA + Qwen-Image-2.1 文生图 D 臂，目标质检后 1085 张；目前只有 LoRA 与本机少量试生成，D 的 YOLO 结果尚不存在。现有正文、9 表和 6 图是 v4 三臂版本，更新前不能标为四臂结论。
...
**图表素材清单（2026-09-25 更新）**：现有图 1–6 和 9 张表均按 v4 三臂生成；图 1 管线、图 2 样例、图 3 选权、图 4 CI、图 5 定性检出、图 6 规模曲线。D 臂生成、标注与评估完成后，按实际证据决定是否增补四臂样例、主表与配对区间；先处理现有图表浮动和 9 表超出目标 4–6 表的问题。不得仅把图题中的“三臂”改为“四臂”。
