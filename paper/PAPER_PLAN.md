...
1. **Introduction**——工地危险样本稀缺是趋势性问题、摆拍有安全与伦理问题、**编辑合成（state-level augmentation）**是出路；结尾放显式 contributions 列表（C1 编辑合成 + 预打标 + 人工复核的闭环管线；C2 来源可追溯的干净评测协议与泄露审计；C3 与经典增广的**同源对照** + 规模效应的实证；C4（第二目标完成后）跨目标通用性）。
...
3. **Methodology**——3.1 问题与类别定义（监理口径：放置问题=未固定，含倒地）；3.2 真实种子数据（93 张人工标注；标注单位是框、图上可混合）；3.3 **编辑合成管线**（Qwen-Image-Edit-2509 图生图编辑真实巡检照片，合成"违规状态"；真实性红线：脏污/磨损，防"过净"域差；**来源记录与审计**）；3.4 自动预打标 + 人工复核闭环（labeler 模型 + annotator GUI）；3.5 评测协议（**独立 test = 61 张网络图**，不参与任何训练来源；固定 100 轮 / imgsz 640 / batch 16、报 last.pt、不做选权；六权重统一超参）。
...
**图表素材清单**（可先攒）：图 1 管线总图（必画，Methodology 骨架）；编辑合成样本拼图（展示脏污真实感 + 与源照片对照）；**来源审计示意**（合成图来源链 → test 独立性，v2 泄漏教训写成方法学贡献）；三臂结果表（六权重 × 新 test）；**规模曲线（394 vs 868，已有数据）**；imgsz 扫描曲线；新 test 检出示例（TP/FP/FN）；【待】安全帽样本与结果。
...
| Experiments 4.3 规模消融 | ✅ 可写（394 vs 868 完成，六权重全涨） | `phase10_v3_main.json` |
...
2. **数据规模消融**【✅ 已完成 2026-09-19】——gen493（train 394）vs gen1085（train 868）在独立新 test 上：六权重 mAP50-95 **全涨 +0.07~+0.15**（yolo26s 0.557→0.698）。「扩量有效」成立，且有第二点。
3. **消融与稳健性**（见 `List_of_Experiments.md` 第二节）：复核价值（autolabel vs 复核标签）、空标图消融、多随机种子、工作点 PR、**分辨率敏感性**（新 test 图分辨率远小于训练图，已确认要补）、误差分析图。
4. **安全帽第二目标**（`Hardhat\` 素材已起步）——跨目标证据，支撑方法通用性；样本到手后的实验（含跨目标迁移、双目标统一模型）见清单第三节。
5. **公开数据对照**：两个 Roboflow 气瓶数据集已查证无放置位姿标注、不可用，改作 Introduction/Discussion 论据（\cite{rfcylinders, rfgarrafas}）。
...
- **把 v2 泄漏事故写成方法学贡献**：合成数据必须可追溯生成来源、测试集图片不得作为生成源——给出审计协议与工具，并用"审计前后数字对比"说明不这么做的代价。
- **三级定位**：
