...
1. **Introduction**——工地危险样本稀缺是趋势性问题、摆拍有安全与伦理问题、**编辑合成（state-level augmentation）**是出路；结尾放显式 contributions 列表（C1 编辑合成 + 预打标 + 人工复核的闭环管线；C2 **与源数据完全无关的独立 test + 可复现的独立性检验**（缩略图相关性，非"来源审计"）；C3 与经典增广的**同源对照** + 规模效应的实证；C4（第二目标完成后）跨目标通用性）。
...
2. **数据规模消融**【✅ 已完成 2026-09-19】——gen493（train 394）vs gen1085（train 868）在独立新 test 上：六权重 mAP50-95 **全涨 +0.07~+0.15**（yolo26s 0.557→0.698）。「扩量有效」成立，且有第二点。⚠️ **协议脚注**：gen493 六权重是 v1 时期训练的（`patience=100`、其 data.yaml 的 test 指 v1_split），只把评测统一到 v3；已核六权重都跑满 100 轮、早停未触发，故对比成立。**待办**：按 v3 协议重跑 gen493（见 `List_of_Experiments.md` 第一节末行），重跑后本表数字以新值为准。
...
- **独立性主张走"换 test + 可复现检验"**：合成数据的编辑源就是真实种子照片，所以**不能**拿同源真实照片当 test（v1/v2 的教训）；v3 直接换用与源数据无关的外部 test（61 张网络图），并用缩略图相关性把它变成一次可跑的检查（`scripts\check_test_independence.py`）。**不要再写"来源审计 / 生成来源可追溯协议 / 611 张泄露"那套 v2 末期废案**——它已被整体废弃，见 `PROGRESS.md` 三节末。
- **三级定位**：
